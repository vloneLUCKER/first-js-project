...
})({"fJNJr":[function(require,module,exports) {
...
module.bundle.HMR_BUNDLE_ID = "358865fc065ab792";
"use strict";
...
},{}],"3gxOf":[function(require,module,exports) {
...
const modalWindowCloseBtn = document.querySelector('[data-action="close-modal"]');
...
modalWindowOpenBtn.addEventListener("click", ()=>{
    modalWindowEl.addEventListener("click", (e)=>{
        if (e.target === e.currentTarget) document.body.classList.remove("show-modal");
    });
    window.addEventListener("keydown", (event)=>{
        if (event.key === "Escape") document.body.classList.remove("show-modal");
    });
    document.body.classList.add("show-modal");
});
modalWindowCloseBtn.addEventListener("click", ()=>document.body.classList.remove("show-modal"));
// const onClick = (e) => {
...
const onClick = (e)=>{
    ...
    if (selectedBtns.has(e.target.dataset.value)) selectedBtns.delete(e.target.dataset.value);
    else selectedBtns.add(e.target.dataset.value);
...
const createListItemMarkup = (items)=>items.map((item)=>`<li>${item.label}</li>`).join(``);
const onKeyPress = function(e) {
    const savedMsg = e.target.value.toLowerCase();
    const filteredItems = items.filter((item)=>item.label.toLowerCase().includes(savedMsg));
    document.querySelector(".js-list").innerHTML = filteredItems.map((item)=>`<li>${item.label}</li>`).join(``);
...
},{}]},["fJNJr","3gxOf"], "3gxOf", "parcelRequire6dcf")

//# sourceMappingURL=02-properties.065ab792.js.map
